...
| **gpt** | Ask GPT (5.6 family, via Codex CLI on your ChatGPT account) as a second-opinion subagent — thinking-level control (`--think minimal→high`), blind `--vs` comparisons sealed before GPT speaks, `--here` repo-aware mode. Opinions labeled, never treated as sources. |
